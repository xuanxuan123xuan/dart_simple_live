...
**dev 分支正在验证的内容**：
- **应用内更新检查**：关于页可检查 GitHub Release，区分 dev / stable 发布通道
- **关注页体验优化**：关注卡片布局、内容裁切、文本导入导出入口、特别关注显示开关和刷新性能已调整
- **抖音搜索兼容**：补充兼容兜底与提示，降低接口变化导致的空结果误判
- **快手关注状态修复**：修复关注状态路径、账号详情回填、播放页状态同步和安全验证提示
- **上游稳定性修复合并**：同步上游 v1.12.8 稳定性改动，并在 dev 线继续回归；感谢 [@June6699](https://github.com/June6699)

以上为 dev 相对 stable `1.13.2` 的增量验证项；已合入 stable 的 Flutter 3.41、低延迟与网络自愈、快手基础治理、跨平台聚合搜索和移动播放体验不再重复列入。
...
| 功能 | 说明 | 状态 | 设计文档 |
|---|---|---|---|
| 播放稳定性与延迟优化 | 协议感知缓存、mpv 追帧、自动选线、线路记忆和播放遥测 | 已实施，待长时/弱网验收 | - |
| 快手请求与冷启动治理 | 请求协调、会话复用、限流熔断、稳定播放停轮询、弹幕重试收敛 | 已实施，待真实风控环境验收 | - |
| 网络波动自动诊断修复 | 换房会话隔离、缓冲边沿计数、真实播放端点分层诊断 | 已实施，待弱网真机验收 | - |
| 跨平台聚合搜索 | 一个搜索框同时搜五大平台，结果按平台分区聚合展示；单站页提供完整分页 | 已实施（路线 A）；TV 已同步 core 竞态治理，测试受 native asset 工具链阻塞 | - |
| TV 端多开同屏 | TV-Windows / Android TV 补齐多开入口、焦点控制、播放器生命周期和性能降级 | 评估中 | [TV 端产品定位与功能缺口评估](docs/TV端产品定位与功能缺口评估.md) |
| Twitch 平台接入 | 作为第六个平台接入，先确认 API、OAuth、播放和聊天边界 | 构思阶段 | [Twitch 平台接入构思大纲](docs/Twitch（老鼠台）平台接入构思大纲.md) |
| 竖屏直播弹幕 | 竖屏直播左下角弹幕列表：从下往上、≤半屏、背景透明、长文本自动换行、上滑可回看历史、底部避让控制栏 | 待做 | - |
